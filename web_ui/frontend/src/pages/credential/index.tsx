import { Eye, EyeOff, Plus, Pencil, Trash2, Zap, Loader2, Check, X } from 'lucide-react';
...
import { CustomModelDialog } from '@/components/dialog/CustomModelDialog';
...
function ModelCardItem({
	...
	const handleTest = async () => {
		...
		setTestInfo('');
		try {
			const res = await modelApi.testConnectivity({
				credential_id: credentialId,
				model_name: model.name,
			});
			if (res.success) {
				setTestState('ok');
				setTestInfo(`${res.latency_ms}ms`);
			} else {
				setTestState('fail');
				setTestInfo(res.error ?? 'Unknown error');
			}
		} catch (e) {
			setTestState('fail');
			setTestInfo(e instanceof Error ? e.message : 'Request failed');
		}
	};
		...
		<Card className="shadow">
			<CardHeader>
				...
				<CardAction className="flex items-center gap-1">
					{reasoning && <Badge variant={'outline'}>{t('credential.reasoning')}</Badge>}
					<Badge variant={model.is_custom ? 'default' : 'secondary'} className="text-[10px]">
						{model.is_custom ? t('credential.custom') : t('credential.builtIn')}
					</Badge>
			...
			<CardContent className="flex flex-col">
				...
				<div className="flex justify-between items-center text-[14px] pt-1">
					<span className="text-muted-foreground">{t('credential.connectivity')}</span>
					<Button
						size="sm"
						variant={
							testState === 'ok'
						...
						title={testState === 'fail' ? testInfo : undefined}
					>
						{testState === 'testing' && <Loader2 className="size-3 animate-spin" />}
						{testState === 'ok' && <Check className="size-3 text-green-500" />}
						{testState === 'fail' && <X className="size-3" />}
						{testState === 'idle' && <Zap className="size-3" />}
						{testState === 'testing'
...
function DetailPanel({ credential, schema, onEdit, onDelete }: DetailPanelProps) {
	...
	const [deletingModel, setDeletingModel] = useState<ModelCard | null>(null);
				...
				<>
					...
					<DeleteDialog
						...
						title={t('common.deleteTitle', {
							entity: t('common.model'),
							name: deletingModel?.label ?? deletingModel?.name ?? '',
						})}
						description={
							deletingModel?.is_custom
								? t('common.deleteDescription')
								: t('credential.hideDescription')
						}
						onConfirm={async () => {
							if (!deletingModel) return;
							if (deletingModel.is_custom && deletingModel.id) {
								await modelApi.deleteCustom(deletingModel.id);
							} else {
								await modelApi.hideBuiltin(type, deletingModel.name);
							}
							fetchModels();
						}}
					/>
				...
				<SidebarContent>
					{loading ? (
						...
						groupedByType.map(({ type, title, records }) => (
							<SidebarGroup key={type} className="py-1">
