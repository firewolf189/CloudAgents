...
export function ToolCallGroupList({
	...
	return (
		<div className="flex flex-col w-full min-w-0 overflow-hidden">
			<div className="flex flex-row gap-x-2 w-full min-w-0 items-center">
				<ToolStateIcon states={calls.map((item) => item.result?.state)} />
				{label}
			</div>
			<div className={`flex ${inline ? 'flex-row' : 'flex-col'} gap-x-2 pl-6 min-w-0 overflow-hidden`}>
				{calls.map((item, index) => (
					<div
						key={item.call.id}
						className="flex flex-row gap-x-2 w-full min-w-0 items-stretch overflow-hidden"
					>
