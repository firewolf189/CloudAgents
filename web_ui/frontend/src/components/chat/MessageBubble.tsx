...
function renderBlock(
	...
	switch (block.type) {
		case 'tool_call_group': {
			...
			return (
				<div key={index} className="flex flex-col gap-y-4 text-muted-foreground min-w-0 overflow-hidden">
		...
		case 'text':
			return (
				<div key={index} className="prose w-full min-w-0 max-w-none overflow-hidden">
					<ReactMarkdown
...
export function MessageBubble({ message, onUserConfirm }: MessageBubbleProps) {
	...
	return (
		<div
			className={`flex w-full gap-3 mb-4 overflow-hidden ${isUser ? 'flex-row-reverse' : 'flex-row'}`}
		>
			{/* Avatar */}
			<div
			...
			{/* Content column */}
			<div
				className={`flex flex-col min-w-0 overflow-hidden ${isUser ? 'items-end max-w-[75%]' : 'items-start flex-1'}`}
			>
				{showBody && (
					<div
						className={`p-3 space-y-2 overflow-hidden ${
							isUser
								? 'w-fit max-w-full rounded-2xl rounded-tr-sm bg-blue-500 text-white [&_.prose]:text-white [&_.prose_strong]:text-white [&_.prose_code]:text-blue-100 [&_.prose_code]:bg-blue-600/50 [&_.prose_a]:text-blue-100'
								: 'w-full rounded-2xl rounded-tl-sm bg-muted/60'
						}`}
					>
						{blocks.map((block, i) =>
							renderBlock(
				...
				{showFooter && (
					<div className="flex flex-row items-center text-muted-foreground gap-x-4 px-1 mt-1 w-full">
						<Badge
							variant="secondary"
